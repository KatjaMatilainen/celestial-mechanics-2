...
;I usually use this function, but now it's not compatible, gives error:
;% Keyword RETAIN not allowed in call to: DEVICE
;% Execution halted at: PLANETANDSATELLITES  116
;------------------------------------------------------------------------;
...
;	          FOR THE CARTESIAN COMPONENTS
	...
	dzdt = ww
        ...
        Rp2=Rp*Rp
        helper1=3.d0*j2*mu*Rp2/(2.d0*r4)*(5.d0*zz*zz/r3-1.d0/rr)
        helper2=3.d0*j2*mu*Rp2/r5

	dudt = -1d0*(mu/r3)*xx+helper1*xx
	dvdt = -1d0*(mu/r3)*yy+helper1*yy
	dwdt = -1d0*(mu/r3)*zz+helper1*zz-helper2*zz
...
;------------------- MAIN -----------------------
;------------------------------------------------
pro advanced,noj2=noj2

	device,decomposed=0,retain=2
        LOADCT, 14 ;This is for the .gif
	tek_color
	!p.charsize=2.3
	thick=2
	!x.thick=thick
...
;----------------------------------------------------------------------;
;!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!;
;MODIFY Z-DIRECTION STARTING VELOCITY, set to non-zero value
;!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!;
;----------------------------------------------------------------------;
	wstart=0.1d0
...
;IGNORE: Create image for the .gif
;        IMAGE1 = DIST(800)
...
;------------------------------------------------------------------------;
; Testing out if saving to .gif works (it doesn't, I'll try again later)
;------------------------------------------------------------------------;
;  TV, IMAGE1
...
;
		...
		rr=sqrt(r2)
;-------------------------------------------------------------------------;
;THIS WHOLE SECTION DEALS WITH POMEGA
...
;--------------------------------------------------------------------;
;pro Plot_everything
;PsPlot, 'PlanetAndSatellites', 'PlanetAndSatellites.ps'
;end
